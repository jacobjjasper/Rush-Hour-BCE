...
class RushHour(object):
    ...
    def play(self, game):
        """ Show visualization of game being won (hardcoded in txt file).
        Implement moves for the visualisation of a game being won. The moves
        are hardcoded for the purpose of being able to show a winning sequence
        of moves.
        ...
        with open(f"data/win{game}.txt") as file:
            reader = file.readlines()

            for line in reader:
                line = line.strip()
                self.move(int(line.split(",")[0]), int(line.split(",")[1]))
